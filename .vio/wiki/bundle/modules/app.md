...
5. **Register blueprints**: `auth_bp`, `main_bp`, `cars_bp`, `services_bp`, `reports_bp`, `print_bp`, `backup_bp`, `moderator_bp`.
6. **Media route**: `/media/<path:filename>` serves uploaded files (login-required).
7. **Jinja helpers**: `currency` and `srdate` filters, plus context-injected globals (`company`, `FUEL_LABELS`, `ROLE_LABELS`, `PERIOD_LABELS`, `currency_code`, `SERVICE_TYPES`, `SERVICE_TYPE_LABELS`).
...
- Registers blueprints documented in: [Authentication & Users](../files/app/auth.md), [Dashboard & Setup](../files/app/main.md), [Car Management](../files/app/cars.md), [Service Records](../files/app/services.md), [Reports & Analytics](../files/app/reports.md), [Printing & PDF](../files/app/printing.md), [Backup System](../files/app/backup.md), Moderator Panel (`app/moderator/routes.py`)
